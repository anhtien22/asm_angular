...
export class TaskService {
  ...
  getMotTask(id:number=0){
    return this.listTask.find(ta => ta.id==id);
  }
  themTask(ta:Task=<Task>{}){
    this.listTask.push(ta);
  }
  capnhatTask(ta:Task=<Task>{}){
    let index = this.listTask.findIndex(t => t.id==ta.id);
    this.listTask[index]= ta;
  }
  xoaTask(id:number=0){
